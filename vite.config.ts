...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      // Backend services have no CORS configured (see BACKEND_GAPS.md) — this
      // proxy makes every request same-origin from the browser's perspective,
      // Vite forwards it server-side where CORS doesn't apply. Dev-only; a
      // production build needs real CORS or a gateway.
      proxy: {
        // Every real devboard-auth endpoint ends in a trailing slash
        // (`/auth/login/`, `/auth/verify-email/`, ...); the frontend's own
        // `/auth/verify-email` and `/auth/reset-password` page routes
        // (dictated by the links devboard-auth's emails already point at)
